-- egxrkin_pm_core.lua
-- Общий модуль Egxrkin Project Manager: парсер .rpp, JSON, индекс, настройки.
-- Загружается без глобального `reaper` (для тестов): reaper.* только внутри функций.

local M = {}

M.NAMESPACE = 'EGXRKIN_PM'          -- namespace project extstate в .rpp
M.EXT_SECTION = 'EGXRKIN_PM'        -- секция глобального extstate (настройки)
M.INDEX_FILENAME = 'egxrkin_pm_index.json'

M.STATUSES = { 'набросок', 'в работе', 'к миксу', 'готово', 'заморожено' }
